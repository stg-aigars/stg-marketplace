...
| Listing card grid | `ListingSection` (heading, optional `eyebrow` uppercase label, optional `description` muted line below heading, optional `href`/`linkText` for `SectionLink` next to heading, responsive grid of `ListingCard`; `emptyState`, `className` overrides) | `@/components/listings` |
...
- **Mapping table:** `src/lib/accounting/mapping.ts` exports `MAPPING_TABLE: VatMappingEntry[]`, currently 33 type IDs (O.1–O.5, O.7–O.9, I.1–I.5, I.7, I.8, P.1, P.6, P.7, H.1–H.3, C.1–C.12). I.6 is a payload modifier (caller passes `expense_account: '1230'` on an underlying I.x type to capitalize), not a routing entry. **C.12** (added 2026-08-03) posts a Union OSS quarterly VAT payment — mirrors C.11 (domestic PVN payment) but targets the per-country OSS payable (5711 LT / 5712 EE) instead of 5710-09; one emit per consumption member state. See `docs/accounting_conventions.md` §6 for usage and §2–§5 for the VAT/OSS date-convention and rounding-gap background that motivated it. Adding a new type is a `mapping.ts` row + a test — no engine change. First-match-wins routing; mutual-exclusivity test in `dispatcher.test.ts` enforces every event matches exactly one type
...
- 44 notification types with prefixes: `order.`, `comment.`, `dispute.`, `shipping.`, `auction.`, `wanted.`, `dac7.`, `moderation.`, `listing.`, `feedback.`, `message.`, `announcement.`. **Every new prefix MUST ship a paired migration adding it to the `notifications_type_check` regex** — `notify()` swallows CHECK violations silently in its internal try/catch, so a missing prefix breaks the affected feature's in-app bell with zero noisy errors. Migration 119 swept the regex after `feedback.`, `moderation.`, `listing.`, and `message.` had drifted for one or more PRs each; migration 120 adds `announcement.` in the same PR that introduces the prefix.
...
- CSP: PostHog origins (`https://*.posthog.com` wildcard) are in `connect-src` in `src/lib/csp.ts` defensively — the wildcard covers `eu.i.posthog.com`, `eu-assets.i.posthog.com`, and the per-tenant `internal-*.posthog.com` subdomain the in-browser PostHog toolbar phones home to (admin-only; visitor analytics route through the proxied `/ingest` path). The middleware `config.matcher` excludes `/ingest/` — there's a protective comment there explaining why; do not remove it
...
- **Registered events** (each entry annotated with `retention_class`):
  ...
  - Additional registered events (financial / dispute / lifecycle / moderation), each with their `retention_class`:
    ...
    - `bank_closure.recorded` (regulatory) — fires from `recordBankStatementClosing` in `src/lib/accounting/bank-closure-actions.ts` when staff record (or correct) a Swedbank statement closing balance for a bank account in a period (PR #4b). `actorType: 'user'`, `actorId` = staff user. `resourceType: 'bank_statement_closure'`, `resourceId` = `<account_code>:<period_key>` (e.g. `2620:2026-05`). Metadata: `{ account_code, period_key, closing_balance_cents, statement_ref }`. Backs the data-driven, multi-account bank reconciliation in period-close checklist item 2 (`getBankClosureReconciliation`), which compares each `accounts.is_bank_reconcilable` account's GL closing to the recorded statement closing. Writes only allowed while the period is `open`. Regulatory retention — the statement closing is the external anchor for the bank reconciliation.
    ...
    - `order.auto_cancelled.{reason}` (regulatory) — fires from the deadlines cron when a deadline expires (e.g. `response_timeout`, `shipping_timeout`), and from `fulfillCartPayment`'s mid-loop rollback (`reason: 'system'`) when a cart-checkout failure cancels an already-created sibling order. Action string is dynamic; the migration 084 backfill matches via `LIKE 'order.auto_cancelled.%'`. Triggers a refund chain.
