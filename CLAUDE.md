...
- **Error contract for parent RPCs.** PR #5 parent RPCs that `PERFORM public.insert_journal_entry(...)` must catch four distinct error families. The full catchable set:
  ...
  - **Period-not-seeded failure** (migration 094's `enforce_period_status`): SQLSTATE `23503` (foreign_key_violation, used to signal "unknown accounting_period"). Single message: `Unknown accounting_period %; period must be seeded in public.periods (period_type=month)`. Catch via `WHEN foreign_key_violation THEN ...`. Surfaces if the engine ever passes a period_key outside the seeded `2025-05 → 2030-12` window
