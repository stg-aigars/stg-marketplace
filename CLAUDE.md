...
- **Registered events** (each entry annotated with `retention_class`):
  ...
  - Additional registered events (financial / dispute / lifecycle / moderation), each with their `retention_class`:
    ...
    - `order.delivery_escalated` (regulatory) — fires from the deadlines cron in `order-deadlines.ts` when a 21-day no-delivery window auto-creates a dispute. Same regulatory weight as `dispute.opened`.
    ...
    - `order.parcel_returning` (operational) — fires from the tracking sync when a parcel is returning to sender. Companion `dispute.opened` event (regulatory) fires alongside if the auto-escalation triggers.
