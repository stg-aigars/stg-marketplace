...
- **Registered events** (each entry annotated with `retention_class`):
  ...
  - Additional registered events (financial / dispute / lifecycle / moderation), each with their `retention_class`:
    ...
    - `order.parcel_returning` (operational) — fires from the tracking sync when a parcel is returning to sender and triggers an auto-escalation. The auto-escalation also inserts a dispute row and fires a regulatory `dispute.opened` companion against it; the operational `order.parcel_returning` row is the tracking-side trigger record (30-day retention is sufficient because the canonical dispute audit lives on `dispute.opened`).
