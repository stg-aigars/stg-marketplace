...
- O.x order-completion entries are 6 lines, not the 4-line "invoice slice" v3 §A.1–§A.5 currently shows. Full shape per `docs/legal_audit/accountant-completion-entry-signoff.md` (v1.2, accountant-signed 10 May 2026): Dr 5590 suspense gross_cart / Cr 5351 seller_net / Cr 5410-UN unisend_cost / Cr 6310-C commission_net / Cr 6310-S shipping_net / Cr {5710-LV-OUT | 5710-RC-EUSALES omitted | 5711 | 5712} vat_amount. **Decomposition is VAT-inclusive**: commission and shipping are gross; net is extracted via `round_half_up(gross / (1 + vat_rate))` at cent boundary; `vat_amount = commission_vat + shipping_vat`; `seller_net = item_value − commission_gross − unisend_cost` (no separate VAT subtraction because VAT is inside commission_gross). Seller wallet sees only the gross commission deduction. The shipping-expense recognition (Dr 7720 / Cr 5310-UN) fires later via I.1 vendor invoice, not at completion. Required payload keys for O.1–O.5: `item_value_cents`, `shipping_value_cents`, `unisend_cost_cents`, `invoice_number`. PR #5 lifecycle integration is when `buildOrderRevenueLines` reworks 4 → 6 lines AND switches from VAT-exclusive (PR #2 idle slice) to VAT-inclusive math; pre-PR-#5 implementation was idle (Phase 0 backfill produced zero O.x entries). v2 §K.4's worked example uses the superseded VAT-exclusive math (seller_net €83.35 for LV €100/€5); under v1.2 the same example yields seller_net €86.50 — v2 reconcile queued non-blocking.
