...
- **Error contract for parent RPCs.** PR #5 parent RPCs that `PERFORM public.insert_journal_entry(...)` must catch four distinct error families. The full catchable set:
  - **Caller-input failures from the RPC itself** (migration 097): SQLSTATE `P0001` (default for `RAISE EXCEPTION` without `USING ERRCODE`), message-prefixed. Three families: `POSTING:MISSING_KEY <field>` (required jsonb key absent), `POSTING:INVALID_SHAPE <reason>` (p_lines not array, < 2 lines, p_entry not object), `POSTING:INVALID_VALUE <field> <reason>` (typed cast failure). Catch via `WHEN sqlstate 'P0001' THEN IF SQLERRM LIKE 'POSTING:%' THEN ...`
