...
- O.x order-completion entries are **7 lines**, not the 4-line "invoice slice" v3 §A.1–§A.5 currently shows. Full shape per `docs/legal_audit/accountant-completion-entry-signoff.md` (v1.3, accountant-signed 11 May 2026): Dr 5590 suspense gross_cart / Dr 7720 unisend_cost / Cr 5351 seller_net / Cr 5410-UN unisend_cost / Cr 6310-C commission_net / Cr 6310-S shipping_net / Cr {5710-LV-OUT | 5711 | 5712} vat_amount (Line 7 omitted for B2B RC O.2/O.4). **Two methodological choices both load-bearing:** (1) VAT-inclusive decomposition: commission and shipping are gross; net is extracted via `round_half_up(gross / (1 + vat_rate))` at cent boundary; `line_vat = gross − line_net` (NEVER computed independently — produces sub-cent disagreement). (2) Agency / principal model: STG is the principal in the shipping contract with Unisend (not pass-through). Unisend cost recognized as STG expense (Dr 7720) at completion, paired with Cr 5410-UN accrued liability — washed against vendor's I.1 invoice (PR #4b territory). **`seller_net = item_value − commission_gross`** (under v1.3, NOT minus unisend_cost — STG bears it as expense, not seller via wallet deduction). Required payload keys for O.1–O.5: `item_value_cents`, `shipping_value_cents`, `unisend_cost_cents`, `invoice_number`. PR #5 commit 6 reworks `buildOrderRevenueLines` from 4 → 7 lines (3 additive: Dr 5590, Dr 7720, Cr 5410-UN); pre-PR-#5 implementation was idle (Phase 0 backfill produced zero O.x entries). v2 §K.4 (seller_net €83.35 under VAT-exclusive seller-bears) and v1.2 (€86.50 under VAT-inclusive seller-bears) are both superseded by v1.3 (€90.00 under VAT-inclusive STG-bears).
