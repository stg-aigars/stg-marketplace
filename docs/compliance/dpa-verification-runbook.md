...
| - | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | **EveryPay (Maksekeskus AS)** | Processor — payments | ⏳ | `…/everypay.pdf` | n/a (EEA) | — | `…/everypay-subprocessors-YYYY-MM-DD.pdf` | n/a (EEA) | EE confirmed | EEA-resident Estonian PI. Maksekeskus AS rebranded to EveryPay AS — same legal entity, same reg. number 12280690 — historical row label preserved to align with the artifact filenames. Technical Provider engaged by Swedbank AS under §1 + §2.8 of the E-commerce Payments Platform T&Cs. Art. 28 framework via §12 + Principles of Processing Personal Data document on swedbank.lv. No SCCs needed. Verify DPA is in the MSA or appended as addendum. Check whether their sub-processor list includes any US-parented entities (card networks, fraud providers). |
