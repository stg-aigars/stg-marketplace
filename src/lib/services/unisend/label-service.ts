...
export async function generateShippingLabel(
  ...
  const parcelRequest: CreateParcelRequest = {
    ...
    sender: {
      ...
      address: { countryCode: params.senderCountry },
      contacts: { phone: params.senderPhone },
    },
    receiver: {
      ...
      contacts: { phone: params.receiverPhone },
    },
    parcel: {
      type: 'T2T',
      size: params.parcelSize,
      weight: params.parcelWeight || 2,
    },
  };
...
export async function downloadLabelFromStorage(orderId: string): Promise<Blob | null> {
  ...
  const { data: files, error: listError } = await supabase.storage
    ...
    .list('shipping-labels', { search: orderId });
